@if (session('status'))
    ...
    >
        <div class="flex min-w-[320px] items-center gap-4 rounded-[1.5rem] border border-white/60 bg-white/90 p-4 shadow-[0_24px_48px_-12px_rgba(15,23,42,0.25)] backdrop-blur-xl">
            ...
            <button @click="show = false" class="text-slate-400 hover:text-slate-600 transition-colors cursor-pointer">
...
@if (session('error'))
    <div 
        ...
        class="fixed bottom-8 left-1/2 z-50 -translate-x-1/2 sm:left-auto sm:right-8 sm:translate-x-0"
    >
        <div class="flex min-w-[320px] items-center gap-4 rounded-[1.5rem] border border-rose-100 bg-white/90 p-4 shadow-[0_24px_48px_-12px_rgba(15,23,42,0.25)] backdrop-blur-xl">
            <div class="flex size-10 items-center justify-center rounded-xl bg-rose-50 text-rose-600 shadow-sm">
            ...
            <div class="flex-1 pr-4">
                <p class="text-sm font-semibold text-slate-900">Gagal!</p>
                <p class="text-xs font-medium text-slate-500">{{ session('error') }}</p>
            </div>
